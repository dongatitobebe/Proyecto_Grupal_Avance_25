...
void GameWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    if (event->key() == Qt::Key_Return || event->key() == Qt::Key_Enter) {
        ...
        if (mundoSeisActivo) {
            qDebug("ENTER presionado en mapa 6 ");
            return;
        }

        // ← Regresar del mapa 3 al 2
        if (mundoTresActivo) {
            if (zonaTransicionMapaTres && player) {
                ...
                // Seguridad: evitar que se use si zona o player no existen
                qDebug("No se puede regresar del mapa 3 al 2: zona o jugador inválido.");
                return;
            }
        }

        // ← Transición mapa 2 al 4
        if (!mundoTresActivo && !mundoCuatroActivo) {  // Solo en mapa 2
            if (zonaTransicionAMapa4 && player) {
                ...
                // Seguridad: evitar uso si zona o jugador no existen
                qDebug("No se puede pasar al mapa 4: zona o jugador inválido.");
                return;
            }
        }

        // ----- Transición mapa 4 → mapa 2 -----
        if (mundoCuatroActivo && zonaTransicionDesdeMapa4 && player) {
            ...
            if (zona.intersects(caballero)) {
                ...
                cambiarAMundoNuevo(); // Reutilizamos para cargar mapa 2
                mundoCambiado = true;
                player->setPos(100, 520); // Ajusta la posición en el mapa 2
                view->centerOn(player);
                return;
            }
        }

        // ← Transición mapa 2 al 3 con ENTER
        if (!mundoTresActivo && !mundoCuatroActivo && puedeCambiarAMapa3) {
        ...
        // ← Entrar al mapa 5 desde el mapa 3
        ...
        // ← Transición del mapa 4 al mapa 6
        ...
        // ← En caso de estar en mapa 4 u otro lugar y sin zona válida
        qDebug("Enter presionado fuera de zona de transición válida.");
        return;
    }
...
void GameWindow::cambiarAMundoCinco() {
    ...
    QGraphicsRectItem* colision1 = scene->addRect(50, 2700, 300, 30);
    colision1->setBrush(Qt::transparent);  // invisible
    colision1->setPen(Qt::NoPen);
    bloqueadores.append(colision1);
